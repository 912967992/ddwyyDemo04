...
//        System.out.println(samples);
    ...
    @ResponseBody
    public Map<String, Object> addNewRow(@RequestParam int sampleId, @RequestParam(required = false) String username, @RequestParam(required = false) String job) {
        // 如果提供了用户名，进行权限检查
...
//        System.out.println("createdAtString:"+createdAtString);
    ...
    @ResponseBody
    public ResponseEntity<String> deleteProblemById(@PathVariable Long id, @PathVariable String username, @RequestParam(required = false) String job) {
        try {
    ...
    public ResponseEntity<InputStreamResource> exportProblemXlsx(@RequestBody Map<String, List<TestIssues>> request) {
        ...
        try {
            ...
            String[] headers = {
                    ...
                    "缺陷等级", "当前状态", "对比上一版或竞品", "DQE确认回复原因与对策（每个版本的回复请勿删除）",
                    "研发确认回复原因与对策（每个版本的回复请勿删除）" , "DQE责任人", "分析责任人", "改善后风险", "评审结论",
                    "下一版回归测试", "备注"
            };
